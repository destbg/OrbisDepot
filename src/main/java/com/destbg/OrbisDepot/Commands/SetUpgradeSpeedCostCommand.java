...
public class SetUpgradeSpeedCostCommand extends AbstractAsyncCommand {
    private final RequiredArg<List<Integer>> values = withListRequiredArg("values", "", ArgTypes.INTEGER);

    public SetUpgradeSpeedCostCommand() {
        super("set-upgrade-speed-cost", "Sets the Voidheart cost for each speed upgrade tier. Accepts " + (Constants.MAX_UPGRADE_RANK - 1) + " values. Default: 4,8,16,64.");
    }
    ...
    protected CompletableFuture<Void> executeAsync(@Nonnull CommandContext context) {
        ...
        for (int i = 0; i < newValues.size(); i++) {
            Constants.UPGRADE_COST_SPEED[i] = newValues.get(i);
        }
        TranslationUtils.refreshUpgradeDescriptions();
        Main.saveOperatorConfig();
        context.sendMessage(Message.raw(TranslationUtils.format("commands.setUpgradeSpeedCost.success", newValues)));
        return CompletableFuture.completedFuture(null);
    }
}
